...
int err_msg(long chk, const char *msg, int code)
{
  if(!chk) {
    if (symtab.symb_str)
...
void codegen(FILE *outpt, const char* template_file) {
  int i, j, k, ncov;
  char sLine[MXLEN+1];
  char buf[64];
  FILE *fpIO;
...
#else
void parse_ode(char **tmplt_file, char **model_file, char **result_file, char **ncovar, char **auxdir) {
  int argc = 6;
  char *argv[] = {"", *tmplt_file, *model_file, *result_file, *ncovar, *auxdir};
#endif
