...
delete_stan_demo_folder <- function() {
  MODELS_HOME <- system.file('include', 'example-models-master', package = 'stanette')
...
function(model = character(0), 
  ...
  if(MODELS_HOME == "") {
    ...
    if(!file.exists(MODELS_HOME)) {
      writable <- file.access(system.file('include', package = 'stanette'), 
                              mode = 2) == 0
      ...
      if(choices[choice] == "Download to include directory of the rstan package") {
        FILE <- file.path(system.file('include', package = 'stanette'),
