...
unconstrain_pars_stanfit <- function(x, pars, ...) {
  ...
  upars <- apply(pars, 1, FUN = function(theta) {
    stanette::unconstrain_pars(x, .rstan_relist(theta, skeleton))
  })
...
log_prob_upars_stanfit <- function(x, upars, ...) {
  apply(upars, 1, stanette::log_prob, object = x,
...
log_lik_i_upars_stanfit <- function(x, upars, i, parameter_name = "log_lik",
  ...
  out <- numeric(S)
  for (s in seq_len(S)) {
    out[s] <- stanette::constrain_pars(x, upars = upars[s, ])[[parameter_name]][i]
  }
  out
}
