...
.onLoad <- function(libname, pkgname) {
  ...
  # if(FALSE && R.version$major >= 4 && Sys.info()["sysname"] == "Darwin") {   ## debug
...
.onAttach <- function(...) {
  rstanLib <- dirname(system.file(package = "stanette"))
  pkgdesc <- packageDescription("stanette", lib.loc = rstanLib)
...
.onUnload <- function(libpath) {
   ...
   library.dynam.unload("stanette", libpath)
